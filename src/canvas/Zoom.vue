    ...
    methods: {
			...
			onZoomChange (forceRender, animate){
				...
				if(this.zoom != this._lastZoom || forceRender){
					...
					if(this.controller){
						/**
						 * will render async! we need do go over controller to get zoomed model
						 */
						if(this.zoomWithCSS || animate){
							this._startCssZoom();
						} else {
			...
			_startCssZoom (){
				this.logger.log(2,"_startCssZoom", "entry >" + this.zoom + "00%");
				this.zoomAnimationRunning = true;
				
				if(this._preZoomAbsPos){
		
					var orginin = this._preZoomAbsPos.x +"px " + this._preZoomAbsPos.y +"px ";
					this.container.style.transformOrigin= orginin;
					var factor = (this.zoom / this._lastZoom);
					css.add(this.container, "MatcCanvasContainerAnimate");			
					this.container.style.transform ="scale(" + factor + ") ";
					setTimeout(lang.hitch(this, "_endCssZoom"), 350);
				}
			},
			
			_endCssZoom (){
				this.logger.log(3,"_endCssZoom", "entry >" + this.zoom + "00%");
