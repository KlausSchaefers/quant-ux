...
export default {
    ...
    methods: {
		...
		renderSliders:function(parent){
			...
			if(this.box){
				/**
				 * FIXME: Create somehow a preview based on the real widget..
				 */
			}
		...
		renderColors:function(db, tbody){
			var tr = db.tr().build(tbody);
		...
		renderSlider:function(db, tbody, msg){
			var tr = db.tr().build(tbody);
			db.td("", msg).build(tr);
		...
		renderShadowBox:function(h,v,b, parent, spread){
