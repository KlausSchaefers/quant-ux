...
export default {
    ...
    methods: {
			...
			init (){
				...
				this.uploader = this.$new(Uploader, {url : '/rest/images/' + this.model.id});
				this.uploader.placeAt(this.uploadCntr);
				this.uploader.setUploadCallBack(lang.hitch(this,"onFilesSelected"));
				if(this.mode == "public"){
					this.uploader.setEnabled(false);
				}		
					
				this.own(this.uploader.on("uploadDone", lang.hitch(this, "onFilesUploaded")));
				this.own(this.uploader.on("uploadError", lang.hitch(this, "onError", "Something went wrong")));
				this.own(this.uploader.on("uploadStart", lang.hitch(this, "setInfo", "Upload started...")));
				this.own(this.uploader.on("uploadNotEnabled", lang.hitch(this, "onError", "Please register to upload...")));
				
				var txt = db.div("MatcHint MatcMarginTop", "Import Screen").build(this.uploadCntr);
				
				this.help = db.div("MatcFontSmall", "").build(txt);
