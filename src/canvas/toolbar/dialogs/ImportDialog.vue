       ...
       <div v-else>
           ...
           <div v-if="tab=== 'figma'">
               <div class="MatchImportDialogCntr">
                   ...
                   <div v-if="figmaWizardPage === 2">
                       <label>{{ getNLS('dialog.import.figma-select-screens')}}</label>
                    
                       <div v-for="preview in figmaPreviews" :key="preview.id">
                           <img :src="preview.props.figmaImage" width="100"/>
                       </div>
                   </div>
...
export default {
   ...
   methods: {
       ...
       async onContinueFigma () {
           ...
           if (this.figmaSelectedPage && this.figmaSelectedScreens.length === 0) {
               this.figmaWizardPage = 2
               this.hasContinue = true
        
               this.tab = 'progress'
               await this.parseFigmaPreviews(this.figmaAcccessKey, this.figmaUrl, this.figmaModel, this.figmaSelectedPage, this.model.screenSize)
               this.tab = 'figma'
               return 
           }
       ...
       resetFigma () {
           ...
           this.figmaWizardPage = 0
           this.figmaSelectedScreens = []
           this.figmaPreviews = []
       },
       ...
       async parseFigmaPreviews (accessKey, url, fModel, figmaSelectedPage, screenSize, importChildren = false) {
           this.logger.log(-1, 'parseFigmaPreviews', 'enter', url)

           try {
               let fileId = this.getFigmaFileKey(url)
               let figmaService = new FigmaService(accessKey)
               this.setProgress(0, 'dialog.import.figma-progress-preview')
               let figmaPreviews = await figmaService.getPreviews(fileId, fModel, importChildren, screenSize, [figmaSelectedPage])
          
               if (figmaPreviews) {

                   this.figmaPreviews = figmaPreviews
                 
               } else {
                   this.logger.error('parseFigmaPreviews', 'Cannot get previews')
                   this.logger.sendError(new Error('Could not parse figma'))
                   this.errorMSG = this.getNLS('dialog.import.error-figma-load-preview')
                   this.tab = 'figma'
               }
           } catch (err) {
               this.logger.error('parseFigmaPreviews', 'Cannot get previews', err)
               this.logger.sendError(err)
               this.errorMSG = this.getNLS('dialog.import.error-figma-load-preview')
               this.tab = 'figma'
           }
       ...
       async parseFigma (accessKey, url, fModel, figmaSelectedPage, screenSize, importChildren = false) {
           ...
           try {
               ...
               if (model) {
                   ...
                   this.logger.error('parseFigma', 'Cannot parse Figma')
                   this.logger.sendError(new Error('Could not parse Figma'))
                   this.errorMSG = this.getNLS('dialog.import.error-figma-load')
                   this.tab = 'figma'
               }
           } catch (err) {
               this.logger.error('parseFigma', 'Cannot parse Figma', err)
               this.logger.sendError(err)
               this.errorMSG = this.getNLS('dialog.import.error-figma-load')
               this.tab = 'figma'
           }
