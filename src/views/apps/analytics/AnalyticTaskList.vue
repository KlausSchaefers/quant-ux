		...
		<div class="" data-dojo-attach-point="sectionTask"> 
			<div class="row">
				...
				<div class="col-md-3 MatcRight"> 
					<a class="MatcButton " data-nls="btn.download" @click="downloadCVS">Download</a>
				</div>
			</div>
			<div class="MatcMarginTop" data-dojo-attach-point="tableCntr">
			</div>
			
			<div class="row">
				<div class="col-md-12"> 
					<div class="MatcButton" data-nls="testSettingsAddTask" @click="addTaskToTable">Add task</div>
				</div>
			</div>
		</div>
	</div>
...
export default {
    ...
    methods: {
		...
		downloadCVS:function(){						
			...
			var csvContent="Name,Success Rate,Duration (Mean),Duration (Median), Duration (STD),Events (Mean), Events (STD)\n"; 
		...
		render (){
			...
			if(this.test.tasks.length > 0){			
				...
				var msg = this.db.div("MatcHint MatcMarginBottom").build(this.tableCntr);
		...
		renderTaskTable (){				
			...
			var row = db.div("row").build(this.tableCntr);
			var col = db.div("col-md-12").build(row);
		...
		_showTaskPerf (summary, i, node) {			
			...
			if (task) {
				...
				var gram = this.$new(TaskPerfGram,{model:this.model, dialog:dialog, mode: this.mode});
