...
export default {
    ...
    methods: {
        postCreate: function(){
			...
			this.userRing = this.$new(Ring, {width:this.width, size:this.size, color:this.RING_FOREGROUND, backgroundColor : this.RING_BACKGROUND});
			...
			this.covRing = this.$new(Ring,{width:this.width, size:this.size, color:this.RING_FOREGROUND, backgroundColor : this.RING_BACKGROUND});
			...
			db.div("MatcDashLabel", this.getNLS("analytics.header.duration")).build(cntr);
