<template>
     <div class="MatcAnnotation">
		<div data-dojo-attach-point="chkBoxCntr" class="MatcMarginBottomXXL"> 		
		</div>						
	</div>
</template>
<script>
...
export default {
    ...
    methods: {
        postCreate: function(){
			...
			this.btn.setOptions([
			    {label : "The test was ok", value:true, css:"MatcButtonGreen", icon:""},
			    {label : "Ignore this test", value:false, css:"MatcButtonRed", icon:""},
			]);
			...
			this.own(on( this.deleteBtn, touch.press, lang.hitch(this,"deleteSession")));		
		...
		async sendUpdate (){
			if(this.pub){
				...
				if(!this.value.id){
					let res = await Services.getModelService().saveAnnotation(this.appID, this.value)
					this.onAnnotionAdded(res)
				} else {
					let res = await Services.getModelService().updateAnnotation(this.appID, this.value)
					this.onAnnotionAdded(res)
				}
			}
		},
		...
		deleteSession:function(){
			if(this.pub){
				...
				if(this.sessionID && this.appID){
					Promise.all([
						Services.getModelService().deleteAnnotation(this.appID, this.value.id),
						Services.getModelService().deleteEventsBySession(this.appID, this.sessionID),
						Services.getModelService().deleteMouseBySession(this.appID, this.sessionID),
					]).then (() => {
