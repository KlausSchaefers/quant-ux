...
export default class CircleTool extends Tool{
    ...
    constructor (editor) {
        ...
        this.logger = new Logger('CircleTool')
    }
    ...
    onMove (point) {
       if (this._startPos){
            // A circle is an ellipse constrained to a square bounding box,
            // so we always force the box to be square (as if shift was pressed).
            const box = this.getBox(this._startPos, point, true)
            const d = this.path.d
            if (d.length === 0) {
                this.initBox(d)
            }
