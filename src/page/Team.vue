...
export default {
    ...
    methods: {
		...
		async showAdd (){
			...
			this.db.h3("MatcDialogHeader", "Add Team Member").build(right);
			
			var email = this.$new(Input);
			email.setPlaceholder("Enter the email");
			email.setCss("form-control input-lg");
			email.placeAt(right);

			var error = this.db.div("MatcErrorLabel MatcErrorLabelEmpty").build(right);
			
			
			var bar = this.db.div("row").div("col-md-12").div("MatcButtonBar MatcMarginTop").build(cntr);
			...
			let suggestions = await Services.getModelService().findTeamSuggestions(this.appID)
		...
		showEdit:function(item, user){
			...
			var popup = this.db.div("MatcTeamDialog MatcTeamDialogEdit MatcPadding MatcActionBox").build();
			
			var cntr = this.db.div("container").build(popup);
			var row = this.db.div("row").build(cntr);
			
			var left = this.db.div("col-md-4 MatcCenter").build(row);
			this.createUserImage(user,left);
			
			var right = this.db.div("col-md-8").build(row);
			
			var lbl = this.db.div("MatcTeamUserName", this.getUserName(user)).build(right);
			var radio= null;
			...
			left = this.db.div("col-md-4 MatcCenter").build(row);
			right = this.db.div("col-md-8 MatcButtonBar").build(row);
			...
			if(user.permission != 3){		
				...
				s.placeAt(popup);
				s.setOptions([
					{value:"delete", icon: "glyphicon glyphicon-trash", callback:lang.hitch(this, "removeUser", user, radio, d )}
				]);				
		...
		async addUser ( input, error, dialog, bar, e){
			...
			let result = await Services.getModelService().createTeam(this.appID, user)
