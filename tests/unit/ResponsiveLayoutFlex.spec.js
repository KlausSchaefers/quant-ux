...
test('ResponsiveLayout.resizeFlex() - row: fixed child keeps its width, others grow equally, cross axis keeps its own size by default', () => {
    ...
    // no alignItems: stretch, so height stays each child's own h:30
...
test('ResponsiveLayout.resizeFlex() - column: fixed child keeps its height, others grow equally, cross axis keeps its own size by default', () => {
    ...
    // inner box: h 190 (200 minus 2x5 padding)
    // fixed child keeps h:20, the 2 grow children equally split the remaining 160 (190 - 20 - 2*5 gap) -> 80 each
    // no alignItems: stretch, so width stays each child's own w:40
    expect(newNestedPositions.x).toEqual({ x: 5, y: 5, w: 40, h: 20 })
    expect(newNestedPositions.y).toEqual({ x: 5, y: 30, w: 40, h: 80 })
    expect(newNestedPositions.z).toEqual({ x: 5, y: 115, w: 40, h: 80 })
})
