...
test('GridUtil.getFlexContainerLines() - row: one x line half a gap before and after every child', () => {
    ...
    const lines = GridUtil.getFlexContainerLines(cntr, 1)
...
test('GridUtil.getFlexContainerLines() - the gap is zoomed, the children are not', () => {
    ...
    const cntr = {
        ...
        children: [
            child('a', 110, 110, 50, 100)
        ]
    }

    const lines = GridUtil.getFlexContainerLines(cntr, 0.5)
...
test('GridUtil.getFlexContainerLines() - rowReverse/columnReverse use the same axis as row/column', () => {
    ...
    const row = {
        style: { flexDirection: 'rowReverse', gap: 20 },
        children: [child('a', 110, 110, 50, 100)]
    }
    expect(GridUtil.getFlexContainerLines(row, 1).x).toEqual([100, 170])

    const column = {
        style: { flexDirection: 'columnReverse', gap: 20 },
        children: [child('a', 110, 110, 100, 50)]
    }
    expect(GridUtil.getFlexContainerLines(column, 1).y).toEqual([100, 170])
})
...
test('GridUtil.getFlexContainerLines() - no gap means the lines sit on the child edges', () => {
    ...
    expect(GridUtil.getFlexContainerLines(cntr, 1).x).toEqual([110, 160])
})
